    ...
    , ui(new Ui::FaceAttendance)
{
    ...
    //导入级联分类器文件
    if(!cascade.load("/opt/opencv4-pc/share/opencv4/haarcascades/haarcascade_frontalface_alt2.xml"))
    {
...
void FaceAttendance::recv_data()
{
    ...
    //通过样式来显示图片
    ui->headLb->setStyleSheet("border-radius:65px; background-color: #00313236; border-image: url(./face.jpg);");
    ui->widgetLb->show();
}
